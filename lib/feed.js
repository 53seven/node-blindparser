...
exports.parseURL = function(feedURL, options, callback){
		...
		var req = request({uri:feedURL, jar:false, proxy:false, followRedirect:true, timeout:1000*30}, function (err, response, xml) {
		//console.log("req done:" + feedURL);
			if(err || xml == null){
				if(err){
					callback(err,null);
				}else{
				...
				parser.addListener('end',function(err, jsonDOM){
					if(err){
						callback(err, null);
					}else{
						if(jsonDOM){
