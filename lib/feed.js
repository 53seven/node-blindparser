...
function parseURL(feedURL, options, callback) {
  ...
  if (u.protocol == 'http:' || u.protocol == 'https:') {
    ...
    var req = request(options, function(err, response, xml) {
      if (err || xml == null) {
        if (err) {
          ...
          callback(new Error('Failed to retrive source'), null);
        }
      }else {
    ...
    callback(new Error('Only http or https protocalls are accepted'), null);
  }
}
...
function parseString(xml, options, callback) {
  ...
  // xml2js seems to have a problem when nothing but a doctype is specified
  //    so make sure that there is atleast an xml tag
  if (xml.split('<').length >= 3) {
    ...
    parser.addListener('end', function(jsonDOM) {
      if (jsonDOM) {
        ...
        callback(new Error('Failed to parse xml string'), null);
      }
    });
    ...
    callback(new Error('Input xml is malformed'), null);
  }
}
